...
class Toplevel1:
    def __init__(self, top=None):
        ...
        self.Photo = tkinter.PhotoImage(file="download.png")
        ...
        self.Frame2.place(relx=0.056, rely=0.137, relheight=0.812, relwidth=0.893)
    ...
    def showpath(self):
        ...
        self.path = filedialog.askopenfilename(title="select your file", filetypes=[("Micrograph", "*.*")])
    ...
    def get_ml_result(self):
        ...
        if len(self.path) == 0:
            ...
            size = img.size
            if size[0] > 587 or size[1] > 301:
                img = img.resize((587, 301), Image.ANTIALIAS)
            img = ImageTk.PhotoImage(img)
            self.lblmessage.configure(image=img, anchor=tk.CENTER)
            self.lblmessage.image = img

            self.obj_predictor=predictor.TDK_model()
            l=self.obj_predictor.predict(self.path)
            message = " >>> Micrograph Classification : {}".format(l)
            self.Label2.configure(text=message,anchor='nw',justify='left',font = ('Segoe UI',10))
